			...
			var existenciasV=$("#nexistencias").val();
				...
				if (idproductoV!=""){
					if(cantidadV=="" || cantidadV=="." || cantidadV=="0"){
						...
						variables[10]=pesoteoricoV;
						variables[11]=existenciasV;
						variables[12]=stockminimoV;
						variables[13]=stockmaximoV;
						variables[14]=clasificacionV;
	...
	$("#autoidproducto").autocomplete({
		source: function(request, response){
			 $.ajax({
				...
				data: {
					term : request.term,
					idproveedor : $("#idproveedor_ajax").val()
				},
...
function checarCeros(id,idtotal) {
	...
	cost2= parseFloat ($("#cost2"+idtotal).val());
	peso= parseFloat ($("#pesoteoricounitario"+idtotal).html());
	
	if (peso!=0){
		cost2=cost/peso;
	}
	$("#cost2"+idtotal).val(cost2.toFixed(4));
	
	var total=cant*cost;
	var totalpeso=cant*peso;
	$("#monto"+idtotal).text("$"+new Intl.NumberFormat("en-IN").format(total.toFixed(4)));
	$("#pesoteorico"+idtotal).val(totalpeso.toFixed(4));
	
	$("#total"+idtotal).text(total.toFixed(4));
	recorrerTabla("tablaSalida","listaSalida");
}

function checarCeros2(id,idtotal) {
	campo=$("#"+id).val();
	campo=$.trim(campo);
	if (campo=="" || campo==0 || campo=="."){
		$("#"+id).val("0.00");
	...
	cost2= parseFloat ($("#cost2"+idtotal).val());
	peso= parseFloat ($("#pesoteoricounitario"+idtotal).html());
	pesototal= parseFloat ($("#pesoteorico"+idtotal).val());
	if (pesototal!=0){
		cost=cost2*peso;
	}
	$("#cost"+idtotal).val(cost.toFixed(4));
	if (peso!=0){
		cant=pesototal/peso;
	}else{
		cant=$("#cant"+idtotal).val();
	}
	
	if (cant!=0){
		$("#cant"+idtotal).val(cant.toFixed(4));
	}
...
function agregarFila(tabla, elementos,lista){
		...
		while (con < elementos.length){
			...
			if (con==11){ //Existencias
				nuevaFila=nuevaFila+"<td id='existancias"+FILAS+"'>";
					nuevaFila=nuevaFila+elementos[con]; 
				nuevaFila=nuevaFila+"</td>";
			}
			if (con==12){ //Stock minimo
				nuevaFila=nuevaFila+"<td>";
					nuevaFila=nuevaFila+elementos[con]; 
				nuevaFila=nuevaFila+"</td>";
			}
			
			if (con==13){ //Stock maximo
				nuevaFila=nuevaFila+"<td id='stockmaximo"+FILAS+"'>";
					nuevaFila=nuevaFila+elementos[con]; 
				nuevaFila=nuevaFila+"</td>";
			}
			if (con==14){ //Clasificacion
				nuevaFila=nuevaFila+"<td>";
					nuevaFila=nuevaFila+elementos[con]; 
				nuevaFila=nuevaFila+"</td>";
			}
...
function recorrerTabla(tabla,lista){
	...
	$('#'+tabla+' tbody tr').each(function () {
		$(this).find('td').each(function (index,valor) {
			...
			//$(valor).css("background-color", "#ECF8E0");F
		})
		if (cantidad!=0){
...
function llenarSelectEmpleados(condicion){
...
function llenarTabla(){
		...
		$("#loading").show();
		$.ajax({
			url: '../componentes/llenarTabla.php',
			type: "POST",
			data: "submit=&idproveedor="+idproveedor, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
				$("#botonGuardar").show();
				$("#botonProcesar").show();
				$("#botonSave").show();
