...
function calcularTotales(){
	...
	$("#ltotalapagar").html((parseFloat($("#ctotalcompras").val())+parseFloat($("#ctotalgastos").val())).toFixed(2));
	$("#lsaldorestante").html((parseFloat($("#lssaldodisponible").html())-parseFloat($("#ltotalapagar").html())).toFixed(2));
	if(parseFloat($("#lsaldorestante").html()) < 0){
		$("#lsaldorestante").css('color', 'red');
	}
	else{
		$("#lsaldorestante").css('color', 'green');
	}
	$("#lssaldodisponible").css('color', 'blue');
}
...
function eliminarIndividual(id) {
	var encoded = "¿Desea cancelar la autorización del pago?";
...
function eliminarIndividualGasto(id) {
	var encoded = "¿Desea cancelar la autorización del pago?";
	var decoded = $("<div/>").html(encoded).text();
    var pregunta = confirm(decoded);
	if (pregunta){
		eliminar_individual_gasto(id);
	}
}
...
function eliminar_individual_gasto(id){
		$.ajax({
			url: '../eliminar/eliminargasto.php',
			type: "POST",
			data: "submit=&ids="+id, //Pasamos los datos en forma de array
			success: function(mensaje){
				mostrarMensaje(mensaje,id,"eliminar");
