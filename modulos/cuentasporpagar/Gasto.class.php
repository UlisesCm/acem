...
class Gasto{
 //constructor	
	...
	function guardar($idsucursal,$lista,$estatus){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cuentasporpagar']['guardar'])){
			return "denegado";
			exit;
		}
		/////FIN  DE PERMISOS////////
	...
	function actualizar($listaSalida,$listaSalidaGastos){
		
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cuentasporpagar']['modificar'])){
			return "denegado";
			exit;
		}
		$validar="fracaso";
		if($listaSalida!="0"){
			/////FIN  DE PERMISOS////////
			$arregloIdCompra=$this->descomponerArreglo(1,0,$listaSalida);
			$con=0;
			if($this->con->conectar()==true){
				while ($con < count($arregloIdCompra)){
					$varConsulta = "UPDATE compras SET autorizado='AUTORIZADO' WHERE idcompra='$arregloIdCompra[$con]'";
		...
		if($listaSalidaGastos!="0"){
			/////FIN  DE PERMISOS////////
			$arregloIdGasto=$this->descomponerArreglo(1,0,$listaSalidaGastos);
			$con=0;
			if($this->con->conectar()==true){
				while ($con < count($arregloIdGasto)){
					$varConsulta = "UPDATE gastos SET autorizado='AUTORIZADO' WHERE idgasto='$arregloIdGasto[$con]'";
					if(mysqli_query($this->con->conect,$varConsulta)){
						//REGISTRO ACTUALIZADO
						$validar = "exito";
					}
	...
	function ObtenerSaldoDisponible(){
		...
		if($this->con->conectar()==true){
			$varConsulta = "SELECT SUM(saldo) AS sumadesaldos 
			...
			if ($resultado){
				$extractor = mysqli_fetch_array($resultado);
				$Saldo= $extractor["sumadesaldos"];
				return $Saldo;
			}else{
				$Saldo = "ERROR DE CONSULTA";
				return $Saldo;
			}
		}else{
			$Saldo = "ERROR DE CONEXIÓN";
			return $Saldo;
		}
		return $Saldo;
	}
	...
	function obtenerFacturasCompra($idcompra){
		$foliosFacturas="";
		if($this->con->conectar()==true){
			$varConsulta = "SELECT serie, folio 
			FROM archivos 
			WHERE tablareferencia = 'compras' AND idreferencia ='$idcompra' ";
			//echo $varConsulta;
			$resultado=mysqli_query($this->con->conect,$varConsulta);
			while ($extractor=mysqli_fetch_array($resultado)) { 
				$foliosFacturas= $extractor["serie"]."-".$extractor["folio"].",";
			}
			return $foliosFacturas;
		}else{
			$foliosFacturas = "ERROR DE CONEXIÓN";
			return $foliosFacturas;
		}
		return $foliosFacturas;
	}
	...
	function mostrarComprasCancelar($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera){
		/////PERMISOS////////////////
		...
		$consulta = "SELECT 
					...
					compras.autorizado,
					compras.idretiro,
					detallecompras.idproveedor AS idprov 
					FROM compras 
					INNER JOIN detallecompras ON compras.idcompra=detallecompras.idcompra 
					WHERE compras.autorizado = 'AUTORIZADO' AND compras.idretiro = 0 
					GROUP BY idcompra 
	...
	function mostrarCompras($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cuentasporpagar']['consultar'])){
			return "denegado";
			exit;
		...
		$consulta = "SELECT 
					...
					FROM compras 
					INNER JOIN detallecompras ON compras.idcompra=detallecompras.idcompra 
					WHERE compras.autorizado = 'NO AUTORIZADO' 
					GROUP BY idcompra 
					ORDER BY fechavencimiento ASC 
					";
					//echo $consulta;
	...
	function mostrarGastosCancelar($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cuentasporpagar']['consultar'])){
			return "denegado";
			exit;
		...
		$consulta = "SELECT 
					...
					INNER JOIN proveedores ON gastos.idproveedor=proveedores.idproveedor
					INNER JOIN modelosimpuestos ON gastos.idmodeloimpuestos=modelosimpuestos.idmodeloimpuestos
					WHERE gastos.autorizado = 'AUTORIZADO' AND idretiro = 0 
					ORDER BY fechavencimiento ASC   
	...
	function mostrar($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera){
		...
		$consulta = "SELECT 
					...
					INNER JOIN proveedores ON gastos.idproveedor=proveedores.idproveedor
					INNER JOIN modelosimpuestos ON gastos.idmodeloimpuestos=modelosimpuestos.idmodeloimpuestos
					WHERE gastos.autorizado = 'NO AUTORIZADO' 
					ORDER BY fechavencimiento ASC   
					LIMIT $inicial, $cantidadamostrar
					";
					//echo $consulta;
	...
	function ObtenerDatosProveedor($idproveedor){
		...
		if($this->con->conectar()==true){
			...
			if ($resultado){
				$extractor = mysqli_fetch_array($resultado);
				$DatosProveedor= $extractor["nombre"];
				return $DatosProveedor;
			}else{
				$DatosProveedor = "ERROR DE CONSULTA";
				return $DatosProveedor;
			}
		}else{
			$DatosProveedor = "ERROR DE CONEXIÓN";
			return $DatosProveedor;
		}
		return $DatosProveedor;
	}
	...
	function eliminar($ids, $tipoEliminacion){
		...
		if($this->con->conectar()==true){
			if ($tipoEliminacion=='falsa'){
				//REQUIERE CAMPO 'estatus' EN LA TABLA
				if (mysqli_query($this->con->conect,"UPDATE compras SET autorizado ='NO AUTORIZADO' WHERE idcompra IN ($ids)")){
					//BITACORA
					if ($_SESSION['bitacora']=="si"){
	...
	function eliminargasto($ids, $tipoEliminacion){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cuentasporpagar']['eliminar'])){
			return "denegado";
			exit;
		...
		if($this->con->conectar()==true){
			if ($tipoEliminacion=='falsa'){
				//REQUIERE CAMPO 'estatus' EN LA TABLA
				if (mysqli_query($this->con->conect,"UPDATE gastos SET autorizado ='NO AUTORIZADO' WHERE idgasto IN ($ids)")){
					//BITACORA
					if ($_SESSION['bitacora']=="si"){
						$descripcionB="elimin&oacute; falsamente los registros: $ids, de la tabla gastos ";
						$this->registrarBitacora("eliminarFalsa",$descripcionB);
