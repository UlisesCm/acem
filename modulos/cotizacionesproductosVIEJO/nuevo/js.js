...
function agregarFilaOtros(tabla, elementos, lista){
		...
		$("#filasotros").append(nuevaFila); //Coloca la fila al final de la tabla
...
function agregarFila(tabla, elementos,lista){
		...
		while (con <= 7){
			...
			if (con==4){//CANTIDAD
				nuevaFila=nuevaFila+"<td>";
				nuevaFila=nuevaFila+"<input value='"+elementos[con]+"' name='cant"+FILAS+"' type='text' style='text-align:right' class='caja' id='cant"+FILAS+"' onblur=\"checarCeros('cant"+FILAS+"','"+FILAS+"')\" onkeypress=\"return soloNumeros(event,'cant"+FILAS+"');\" />";
				nuevaFila=nuevaFila+"</td>";
			}
			...
			if (con==5){//LISTAS DE PRECIOS
				...
				while(con2 < precios.length){
					...
					nuevaFila=nuevaFila+"<div class='col-md-3 colprecio2'><input class='precios"+IDsListas[con2]+"' id='prec1"+FILAS+"' style='text-align:right' type='radio' name='preciok"+FILAS+"' precio='"+PrecioPublico+"' costopromedio = '"+costopromedio+"' pesoteorico = '"+pesoteorico+"' tasas = '"+Tasas+"' value='"+PrecioNeto+"' onclick=\"checarCeros('prec1"+FILAS+"','"+FILAS+"');CerosAColumnaPrecioUnitario('tablaSalida','"+FILAS+"')\"/>&nbsp;$"+PrecioNeto.toFixed(2)+"</div>";
					con2++;
				}
...
function recorrerTabla(tabla,lista){
	var no, idproducto,cantidad,precio,costopromedio,precioNeto,importe=0,importeNeto,impuestos=0,pesoteorico,utilidad,subtotal=0,sumaimpuestos=0,total=0,costodeventa=0,sumautilidad=0;
	var cadena="";
	$('#'+tabla+' tbody tr').each(function () {
		$(this).find('td').each(function (index,valor) {
			if (index==0){
