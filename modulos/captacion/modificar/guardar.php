...
if (isset($_POST['idcaptacion'])){
	$idcaptacion=htmlentities(trim($_POST['idcaptacion']));
	//$idcaptacion=mysql_real_escape_string($idcaptacion);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idcaptacion no es correcto</p>";
}
...
if($validacion){
	$resultado=$Ocaptacion->actualizar($nombre,$estatus, $idcaptacion);
	if($resultado=="exito"){
