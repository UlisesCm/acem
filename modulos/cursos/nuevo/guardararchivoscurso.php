...
if (isset($_FILES['recurso0']['name'])){
	$archivonombre=$_FILES['recurso0']['name'];
	$archivotemporal=$_FILES['recurso0']['tmp_name'];
	$extencionarchivo=pathinfo($_FILES['recurso0']['name'], PATHINFO_EXTENSION);
	$archivo=basename($_FILES['recurso0']['name'],".".$extencionarchivo)."_".generarClave(5).".".$extencionarchivo;
...
if($archivotemporal!=""){
	...
	if ($estadoArchivo=="exito"){
		...
		echo utf8_encode("fracaso1");
	}else{
		echo utf8_encode("fracaso2");
	}
}
else{
	echo utf8_encode("fracaso3");
}
?>