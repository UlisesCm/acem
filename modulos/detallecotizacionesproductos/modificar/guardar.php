...
if (isset($_POST['iddetallecotizacion'])){
	$iddetallecotizacion=htmlentities(trim($_POST['iddetallecotizacion']));
	//$iddetallecotizacion=mysql_real_escape_string($iddetallecotizacion);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo iddetallecotizacion no es correcto</p>";
}
...
if($validacion){
	$resultado=$Odetallecotizacionproducto->actualizar($subfolio,$idproducto,$cantidad,$costo,$precio,$subtotal,$impuestos,$total,$utilidad,$idcotizacionproducto,$pesounitario,$cantidadentregada, $iddetallecotizacion);
	if($resultado=="exito"){
