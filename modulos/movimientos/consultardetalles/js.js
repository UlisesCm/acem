...
function comprobarReglas(){
	...
	$( ".tipoTabla" ).click(function() {
		...
		$(".tipoLista").show();
		$(".tipoTabla").hide();
		load_tablas(idreferencia,idmovimiento);
	});
...
$(document).ready(function() {
	...
	$(".botonExportar").click(function() {
		var busqueda=$.trim( $("#cajaBuscar").val());
		exportar(idreferencia, idmovimiento);
	});
...
// Onixbm 2014
function load_tablas (idreferencia, idmovimiento){
	//alert (orden);
	//alert (campoOrden);
	//alert (limit);
	...
	xmlhttp.open("POST","consultar.php?idmovimiento="+idmovimiento+"&idreferencia="+idreferencia+"&tabla="+tabla, true);
	xmlhttp.send();
}

function exportar(idreferencia, idmovimiento){
	
	window.open("consultar_excel.php?idreferencia="+idreferencia+"&idmovimiento="+idmovimiento+"&tabla="+tabla);
}
