...
function llenarSelectSucursal(condicion){
		...
		$.ajax({
			...
			success: function(mensaje){
				...
				llenarSelectSerie($("#idsucursal_ajax").val());
			}
		});
		return false;
}

function llenarSelectSerie(condicion){
		$("#idserie_ajax").html("<option value='1'>cargando...</option>");
		$.ajax({
			url: '../componentes/llenarSelectSerie.php',
			type: "POST",
			data: "submit=&condicion="+condicion, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
				$("#idserie_ajax").html(mensaje);
			}
		});
		return false;
}
...
$(document).ready(function() {
	...
	$("#idsucursal_ajax").change(function(){
		llenarSelectSerie($("#idsucursal_ajax").val());
	});
	
	$("#idserie_ajax").change(function(){
		var tiposerie=$("#idserie_ajax option:selected").attr("tiposerie");
		$("#tiposerie").val(tiposerie);
	});
...
function load_tablas (campoOrden, orden, cantidadamostrar, paginacion, busqueda, tipoVista){
	...
	/*var idcliente, idalmacen, tipo, ticket, fitlarfecha, fechainicio, fechafin;
	idcliente=$("#cidcliente").val();
	idsucursal=$("#idsucursal_ajax").val();
	...
	xmlhttp.open("POST","consultar.php?campoOrden="+campoOrden+"&orden="+orden+"&cantidadamostrar="+cantidadamostrar+"&paginacion="+paginacion+"&busqueda="+busqueda+"&tipoVista="+tipoVista+"&papelera="+papelera+"&idcliente="+idcliente+"&idsucursal="+idsucursal+"&ticket="+ticket+"&tipo="+tipo+"&filtrarfecha="+filtrarfecha+"&fechainicio="+fechainicio+"&fechafin="+fechafin+"&diacobro="+diacobro+"&rfccliente="+rfccliente, true);*/
...
function facturar(campoOrden, orden, cantidadamostrar, paginacion, busqueda, tipoVista){
	...
	rfccliente=$("#crfccliente").val();
	alert(rfccliente);
	if (idcliente!="" || rfccliente!=""){
	window.open("general_facturar.php?campoOrden="+campoOrden+"&orden="+orden+"&cantidadamostrar="+cantidadamostrar+"&paginacion="+paginacion+"&busqueda="+busqueda+"&tipoVista="+tipoVista+"&papelera="+papelera+"&idcliente="+idcliente+"&idalmacen="+idalmacen+"&ticket="+ticket+"&tipo="+tipo+"&filtrarfecha="+filtrarfecha+"&fechainicio="+fechainicio+"&fechafin="+fechafin+"&diacobro="+diacobro+"&rfccliente="+rfccliente, true);
	}else{
		mostrarMensaje("fracaso@Operaci&oacute;n fallida@Debe filtrar los resultados seleccionando un cliente en específico.");
	}
	//xmlhttp.send();
}
