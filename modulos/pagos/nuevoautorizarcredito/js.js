...
function eliminarIndividual(id) {
	...
	if (pregunta){
		...
		eliminar_individual(id,tipo,idreferencia,tablareferencia);
	}
}

function eliminar_individual(id,tipo){
		$.ajax({
			url: '../eliminar/eliminar.php',
			type: "POST",
			data: "submit=&ids="+id+"&tipo="+tipo+"&idreferencia="+idreferencia+"&tablareferencia="+tablareferencia, //Pasamos los datos en forma de array
