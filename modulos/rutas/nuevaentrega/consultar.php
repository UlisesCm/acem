...
$resultado=$Ocotizacionproducto->mostrar($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera,$idcliente,$filtrarfecha,$fechainicio,$fechafin,"ENTREGA EN SUCURSAL","EN ESPERA",0);
...
if ($tipoVista=="tabla"){ // Si se ha elegido el tipo tabla ?>
	<div class="box-body table-responsive no-padding"> <!-- /.box-body -->
    	<table class="table table-hover table-bordered">
        	<tr>
				...
				<th class="Cdomicilioentrega" style="display:none">Domicilio entrega</th>
                <th class="Ccoordenadas" style="display:none">Coordenadas</th>
				<th class="Cfechaentrega" style="display:none">Fecha de entrega</th>
				<th class="Cprioridad" style="display:none">Prioridad</th>
                <th class="Cpeso">Peso en KG</th>
                <th class="Ctotal">Total</th>
                <th class="Cobservaciones">Observaciones venta</th>
                <th class="Cobservacionesentrega" style="display:none">Observaciones entrega</th>
				<th width="40"></th>
	...
	while ($filas=mysqli_fetch_array($resultado)) { ?>
      		<tr id="iregistro<?php echo $filas['idcotizacionproducto'] ?>">
                ...
                ?>
				<td class="Cdomicilioentrega" style="display:none"><i><small><?php echo  $domicilio; ?></small></i></td>
                ...
                <td class="Cpeso"><?php echo $Ocotizacionproducto->ObtenerPeso($filas['idcotizacionproducto']); ?></td>
                <td class="Ctotal"><?php echo $filas['total']; ?></td>
				<td class="Cobservaciones"><?php echo $filas['observaciones']; ?></td>
                <td class="Cobservacionesentrega" style="display:none"><input type="text" name="observacionesentrega" id="observacionesentrega<?php echo $con?>" class='caja' value="<?php echo $filas['observacionesentrega']; ?>"/></td>
                <td>
                	<?php
                	/////PERMISOS////////////////
                	if (isset($_SESSION['permisos']['cotizacionesproductos']['consultar'])){
					?>
						<form target="_blank" action="../../detallecotizacionesproductos/consultaagrupada/vista.php?n1=logisticaysalidas&n2=entregasensucursal&n3=nuevorutas" method="post">
                			<input type="hidden" name="idcotizacionproducto" id="idcotizacionproducto" value="<?php echo $filas['idcotizacionproducto'] ?>"/>
                            <button type="submit" class="btn btn-success btn-xs" value="" title="Entregar"><li class="fa fa-truck"></li></button>
                		</form>
                	<?php 
					}else{ ?>
                ...
                ?>
                    <?php if($filas['idcotizacionproducto']==0){ ?>
                        <a class="btn btn-success btn-xs disabled"><i class="fa fa-check-squaren"></i></a>
                    <?php }else{ ?>
                        <a class="btn btn-success btn-xs" id="botonentregar<?php echo $con?>" title="Marcar como entregada" onclick="(actualizarVenta(<?php echo $filas['idcotizacionproducto'] ?>,<?php echo $con ?>))"><li class="fa fa-check-square"></li></a>
                    <?php }?>
                <?php 
                }else{ ?>
                    <a class="btn btn-success btn-xs disabled"><i class="fa fa-check-square"></i></a>
                <?php
                }
                ?>
                </td>
