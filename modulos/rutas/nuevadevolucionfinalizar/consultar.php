...
if (isset($_REQUEST['estadoentrega']) && $_REQUEST['estadoentrega'] !="") {
	if($_REQUEST['estadoentrega']!="undefined"){
		$estadoentrega = htmlentities($_REQUEST['estadoentrega']);
	}else{
		$estadoentrega="";
	}
}else{
...
if($estadoentrega=="NO ENTREGADO"){
	$ocultarColumna ='style = Display:none';
}
...
if($busqueda == ""){
	if($idcotizacionproducto!=""){														
		$resultado=$Odetallecotizacionproducto->mostrarTotalProductosADevolver($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera, $idcotizacionproducto);
	}
...
if ($tipoVista=="tabla"){ // Si se ha elegido el tipo tabla ?>
	<div class="box-body table-responsive no-padding"> <!-- /.box-body -->
    	<table id="tablaSalida" class="table table-hover table-bordered" >
        	<tr>
				...
				<th class="Ccantidad">Cantidad entregada</th>
                <th <?php echo $ocultarColumna ?> class="Ccantidaddevuelta">Cantidad devuelta</th>
	...
	while ($filas=mysqli_fetch_array($resultado)) { ?>
      		<tr id="iregistro<?php echo $filas['iddetallecotizacion'] ?>">
                ...
                <td class="Cnombreproducto"><?php echo $filas['nombreproducto']; ?></td>
				<td class="Ccantidad"><?php echo $filas['cantidadtotal']; ?></td>
                <td <?php echo $ocultarColumna ?> class="Ccantidaddevuelta"><input type="text" name="cantidaddevuelta" id="cantidaddevuelta<?php echo $con ?>" onblur="(recorrerTabla('tablaSalida','listaSalida'))" class='caja' value="<?php if ($estadoentrega=="NO ENTREGADO"){echo $filas['cantidadtotal'];}else{echo "0";}?>"/></td>
                <td class="Ccosto"><?php echo $filas['costo']; ?></td>
                <td class="Cprecio"><?php echo $filas['precio']; ?></td>
                ...
                <td class="Cutilidad"><?php echo $filas['utilidad']; ?></td>
                <td class="Cpesounitario"><?php echo $filas['pesounitario']; ?></td>
