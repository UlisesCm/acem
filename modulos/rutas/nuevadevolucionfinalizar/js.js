...
$(document).ready(function() {
	...
	$("#botonAceptar").click(function(){
		autorizarEntrada($("#idcotizacionproducto").val());
	});
...
function recorrerTabla(tabla,lista){
	var no=0, idproducto,cantidadvendida,cantidaddevuelta,costo,precio,subtotal,impuestos,total,utilidad,pesounitario;
	var cadena="";
	var TotalCantidadDevuelta = 0;
	var Agregar= false;
	$('#'+tabla+' tbody tr').each(function () {
		$(this).find('td').each(function (index,valor) {
			Agregar= true;
			if (index==3){//idproducto
				idproducto=$(valor).html();
			}
			if (index==5){//cantidad vendida
				cantidadvendida=$(valor).html();
			}
			if (index==6){//cantidad devuelta
				cantidaddevuelta=$("#cantidaddevuelta"+no).val();
				if(parseFloat(cantidadvendida) < parseFloat(cantidaddevuelta)){//poner el maximo a cantidaddevuelta
					$("#cantidaddevuelta"+no).val(cantidadvendida);
					cantidaddevuelta=$("#cantidaddevuelta"+no).val();//asignar nuevamente el valor a cantidaddevuelta
				}
				TotalCantidadDevuelta=parseFloat(TotalCantidadDevuelta) + parseFloat(cantidaddevuelta);
				no++;
			}
			if (index==7){//costo
				costo=$(valor).html();
			...
			if (index==10){//impuestos
				impuestos=$(valor).html();
			}
			if (index==11){//total
				total=$(valor).html();
			}
			if (index==12){//utilidad
				utilidad=$(valor).html();
			}
			if (index==13){//peso unitario
				pesounitario=$(valor).html();
			}
		})
		if(Agregar == true){
			cadena=cadena+idproducto+":::"+cantidaddevuelta+":::"+cantidadvendida+":::"+costo+":::"+precio+":::"+subtotal+":::"+impuestos+":::"+total+":::"+utilidad+":::"+pesounitario+":::";
		}
	})
	$("#TotalCantidadDevuelta").val(TotalCantidadDevuelta);
	$("#"+lista).val(cadena);
}
...
function autorizarEntrada(idcotizacionproducto){
	recorrerTabla("tablaSalida","listaSalida");
	var lista = $("#listaSalida").val();
	var estadoentrega = $("#estadoentrega").val();
	var TotalCantidadDevuelta = $("#TotalCantidadDevuelta").val();
	$("#loading").show();
	$.ajax({
		url: 'autorizarDevolucion.php',
		type: "POST",
		data: "submit=&idcotizacionproducto="+idcotizacionproducto+"&lista="+lista+"&TotalCantidadDevuelta="+TotalCantidadDevuelta+"&estadoentrega="+estadoentrega, //Pasamos los datos en forma de array
		success: function(mensaje){
			$("#loading").hide();
