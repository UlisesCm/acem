...
if (isset($_POST['idrutaanterior'])){
	$idrutaanterior=htmlentities(trim($_POST['idrutaanterior']));
	//$idgirocomercial=mysql_real_escape_string($idgirocomercial);
	
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idrutaanterior no es correcto</p>";
}
...
if($validacion){
	...
	$resultado=$Ocotizacionproducto->cancelarLiquidacion($idcotizacionproducto,$estadoliquidacion,$idruta,$idrutaanterior,$estadoentregaanterior);
	if($resultado=="exito"){
