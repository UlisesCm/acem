...
function cancelarLiquidacion(idcotizacionproducto,estadoliquidacion,estadopago,estadoentrega,estadoentregaanterior,envio,idcliente,idruta,idrutaanterior){
	$("#loading").show();
	if(idruta==0){
		//ESTAS DOS SIGUIENTES CONDICIONES NO SE SI REALMENTE ESTAN BIEN
		if(estadoentregaanterior=="ENTREGA PARCIAL"){
			//alert("SE CANCELA NOTA DE CREDITO");
		}
		if(estadoentregaanterior=="NO ENTREGADO"){
			//alert("SE ACTIVA NOTA DE VENTA");
		}
	}
	$.ajax({
		url: 'cancelarLiquidacion.php',
		type: "POST",
		data: "submit=&idcotizacionproducto="+idcotizacionproducto+"&estadoliquidacion="+estadoliquidacion+"&idruta="+idruta+"&idrutaanterior="+idrutaanterior+"&estadoentregaanterior="+estadoentregaanterior, //Pasamos los datos en forma de array
		success: function(mensaje){
			$("#loading").hide();
			mostrarMensaje(mensaje);
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,busqueda,tipoVista);
			if(estadopago =="PAGADO"){//enviar a cuenta por cobrar
...
function llenarSelectRuta(condicion){
		...
		var estadoliquidacion = "LIQUIDADO";
		var fechaini = $("#cfechainicio").val();
		var fechafin = $("#cfechafin").val();
		$.ajax({
			url: '../componentes/llenarSelectRuta.php',
			type: "POST",
			data: "submit=&condicion="+condicion+"&estadoliquidacion="+estadoliquidacion+"&fechaini="+fechaini+"&fechafin="+fechafin, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
