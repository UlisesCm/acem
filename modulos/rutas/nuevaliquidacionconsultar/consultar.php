...
$resultado="";
if($idruta!=""){
	$resultado=$Ocotizacionproducto->mostrarRutaLiquidaciones($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera, $idruta);
...
if ($tipoVista=="tabla"){ // Si se ha elegido el tipo tabla ?>
	<div class="box-body table-responsive no-padding"> <!-- /.box-body -->
    	<table class="table table-hover table-bordered">
        	<tr>
                ...
                <th class="Cordenentrega" style="display:none">Orden entrega</th>
				...
				<th class="Cestadofacturacion" style="display:none">Estado facturación</th>
                ...
                <th class="Ccoordenadas" style="display:none">Coordenadas</th>
                <th class="Ctotal">Total</th>
				<th class="Cobservaciones" style="display:none">Observaciones</th>
                <th class="Cestadopago">Estado pago</th>
                <th class="Cestadocredito">Estado crédito</th>
                <th class="Cestadoentrega">Estado entrega</th>
				<th class="Cestadoliquidacion">Estado liquidación</th>
                <th class="Cobservacionesentrega">Observaciones entrega</th>
                <th class="Cenvio">Envío</th>
                <th class="Cobservacionesliquidacion">Observaciones liquidacion</th>
				<th width="40"></th>
                <th width="40"></th>
	...
	while ($filas=mysqli_fetch_array($resultado)) { ?>
      		<tr id="iregistro<?php echo $filas['idcotizacionproducto'] ?>">
                ...
                <td class="Cordenentrega" style="display:none"><?php echo $filas['ordenentrega']; ?></td>
				...
				?>
				<td class="Cfechaentrega" style="display:none"><i><small><?php echo $nuevaFecha." DE LAS ".$filas['horaentregainicio']." A LAS ".$filas['horaentregafin'];?> </small></i></td>
                ...
                <td class="Ccoordenadas" style="display:none"><?php echo $filas['coordenadas']; ?></td>
                <td class="Ctotal"><?php echo $filas['total']; ?></td>
				<td class="Cobservaciones" style="display:none"><?php echo $filas['observaciones']; ?></td>
                ...
                <td class="Cestadoentrega"><?php if($filas['estadoentregaanterior']!=""){echo $filas['estadoentrega']."/".$filas['estadoentregaanterior'];}else{echo $filas['estadoentrega'];}?></td>
                ...
                <td class="Cobservacionesentrega"><?php echo $filas['observacionesentrega']; ?></td>
                 <td class="Cenvio"><?php echo $filas['envio']; ?></td>
                <td class="Cobservacionesliquidacion"><input type="text" name="observacionesliquidacion" id="observacionesliquidacion<?php echo $con?>" onblur="actualizarObservaciones('<?php echo $filas['idcotizacionproducto']?>','<?php echo $con?>')" class='caja' idcotizacionproducto="<?php echo $filas['idcotizacionproducto'] ?>" value="<?php echo $filas['observacionesliquidacion'] ?>"/></td>
                <td>
                	<?php
                	/////PERMISOS////////////////
        		...
        		<td>
                    ...
                    if (isset($_SESSION['permisos']['cotizacionesproductos']['eliminar'])){
                    ?>
                        <?php if($filas['idcotizacionproducto']==0 || $filas['estadoliquidacion']=="NO LIQUIDADO"){ ?>
                            <a class="btn btn-danger btn-xs disabled"><i class="fa fa-close"> CANCELAR LIQUIDACION</i></a>
                        <?php }else{ ?>
                            <a class="btn btn-danger btn-xs" title="Cancelar liquidación" onclick="(cancelarLiquidacion(<?php echo $filas['idcotizacionproducto'] ?>,'NO LIQUIDADO','<?php echo $filas['estadopago'] ?>','<?php echo $filas['estadoentrega'] ?>','<?php echo $filas['estadoentregaanterior'] ?>','<?php echo $filas['envio'] ?>','<?php echo $filas['idcliente'] ?>','<?php echo $filas['idruta'] ?>','<?php echo $filas['idrutaanterior'] ?>'))"><li class="fa fa-close"> CANCELAR LIQUIDACION</li></a>
                        <?php }?>
                    <?php 
                    }else{ ?>
                ...
                <td style="display:none">
                    ...
                    ?>
                        ...
                        <?php }else{ ?>
                            <a class="btn btn-success btn-xs" title="Liquidado" onclick="(actualizarEstadoLiquidacion(<?php echo $filas['idcotizacionproducto'] ?>,'LIQUIDADO','<?php echo $filas['estadopago'] ?>','<?php echo $filas['estadoentrega'] ?>','<?php echo $filas['envio'] ?>','<?php echo $filas['idcliente'] ?>'))"><li class="fa fa-check">LIQUIDAR</li></a>
                        <?php }?>
                    <?php 
                    }else{ ?>
