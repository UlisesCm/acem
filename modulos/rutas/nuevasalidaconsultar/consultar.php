...
$Oruta=new Ruta;
$condicion="";
$resultado=$Oruta->mostrar($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera, $folio, $filtrarfecha,$fechainicio,$fechafin,$autorizacion);
...
if ($tipoVista=="tabla"){ // Si se ha elegido el tipo tabla ?>
	<div class="box-body table-responsive no-padding"> <!-- /.box-body -->
    	<table class="table table-hover table-bordered">
        	<tr>
				...
				<th class="Cautorizada">Salida</th>
				<th width="40"></th>
                <th width="40"></th>
                <th width="40"></th>
      		</tr>
	...
	while ($filas=mysqli_fetch_array($resultado)) { ?>
      		<tr id="iregistro<?php echo $filas['idruta'] ?>">
				...
				<td class="Cautorizada<?php echo $con?>"><?php echo $filas['autorizada']; ?></td>
        		<td>
                	<?php
                	/////PERMISOS////////////////
                	if (isset($_SESSION['permisos']['cotizacionesproductos']['consultar'])){
					?>
						<form target="_blank" action="../../../modulos/cotizacionesproductos/consultar/vista.php?n1=ventas&n2=cotizacionesproductos&n3=consultarcotizacionesproductos" method="post">
                			<input type="hidden" name="idruta" id="idruta" value="<?php echo $filas['idruta'] ?>"/>
                            <button type="submit" class="btn btn-info btn-xs" value="" title="Detalles (agrupado por ventas)"><li class="fa fa-list"></li></button>
                		</form>
                	<?php 
					}else{ ?>
                ...
                <td>
                	...
                	if (isset($_SESSION['permisos']['cotizacionesproductos']['consultar'])){
					?>
						<form target="_blank" action="../../../modulos/rutas/nuevasalidafinalizar/vista.php?n1=logisticaysalidas&n2=enviosadomicilio&n3=salidas&n4=nuevosalidas" method="post">
                			<input type="hidden" name="idruta" id="idruta" value="<?php echo $filas['idruta'] ?>"/>
                            <input type="hidden" name="hiddenbotonaceptar" id="hiddenbotonaceptar" value="<?php echo 'isplay:none'?>"/>
                            <button type="submit" class="btn btn-success btn-xs" value="" title="Detalles (agrupado por porductos)"><li class="fa fa-list"></li></button>
                		</form>
                	<?php 
					}else{ ?>
                ...
                ?>
                    ...
                    <?php }else{ ?>
                        <a class="btn btn-danger btn-xs" id="botoncancelar" title="Cancelar salida" onclick="(cancelarSalida(<?php echo $filas['idruta'] ?>))"><li class="fa fa-close"></li></a>
                    <?php }?>
                <?php 
                }else{ ?>
                    <a class="btn btn-danger btn-xs disabled"><i class="fa fa-close"></i></a>
