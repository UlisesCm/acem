...
function localizarCoordenadas(coordenada1,coordenada2){
	window.open("https://maps.google.com/maps?q="+coordenada1+","+coordenada2);
	
}

function actualizarEstadoLiquidacion(idcotizacionproducto,estadoliquidacion,estadopago,estadoentrega,estadoentregaanterior,envio,idcliente,idrutaanterior){
	//var autorizacion = $(".Cautorizada"+no).html();
	if(envio == "REENVÍO" && estadoentregaanterior =="NO ENTREGADO"){
	  //no continuar hasta que se cancele la nota
	  alert("NO SE PUEDE LIQUIDAR SE DEBE CANCELAR LA NOTA");
	}
	else{//significa que aún es envio regular O esta marcado como entregado o entrega parcial
		if(envio == "ENVIO REGULAR" && estadoentrega =="DEVUELTO" && estadoentregaanterior =="NO ENTREGADO"){
			//SE DEBE CAMBIAR ENVIO REGULAR POR REENVIO idruta = 0 y idrutaanteriors=252536512536 estadoentrega EN ESPERA en cotizacionesproductos para que vuelva a salir en nueva ruta
			$("#loading").show();
			/*$.ajax({
				url: 'actualizarEnvio.php',
				type: "POST",
				data: "submit=&idcotizacionproducto="+idcotizacionproducto, //Pasamos los datos en forma de array
				success: function(mensaje){*/
					$.ajax({
						url: 'registrarLiquidacion.php',
						type: "POST",
						data: "submit=&idcotizacionproducto="+idcotizacionproducto+"&estadoliquidacion="+estadoliquidacion+"&idrutaanterior="+idrutaanterior, //Pasamos los datos en forma de array
						success: function(mensaje){
							$("#loading").hide();
							mostrarMensaje(mensaje);
							load_tablas(campoOrden,orden,cantidadamostrar,paginacion,busqueda,tipoVista);
							if(estadopago =="NO PAGADO"){//enviar a cuenta por cobrar
						window.open('../../pagos/nuevo/nuevo.php?n1=facturacionycobranza&n2=cuentasporcobrar&idreferencia='+idcotizacionproducto+'&tablareferencia=cotizacionesproductos&idcliente='+idcliente+'&tipo=PRODUCTOS');
							}
						}
					});
					return false;
		...
		if(estadoentrega =="DEVUELTO" && estadoentregaanterior =="ENTREGA PARCIAL"){//SI ES ENTREGA PARCIAL SE EMITE NOTA DE CREDITO
		   alert("SE DEBE EMITIR NOTA DE CREDITO");
		   
		   //CAMBIAR EL ESTATUS DE LIQUIDACION Y ABRIR VENTANA DE PAGOS SI ES NECESARIO
		   if(estadopago =="NO PAGADO"){//enviar a cuenta por cobrar
				window.open('../../pagos/nuevo/nuevo.php?n1=facturacionycobranza&n2=cuentasporcobrar&idreferencia='+idcotizacionproducto+'&tablareferencia=cotizacionesproductos&idcliente='+idcliente+'&tipo=PRODUCTOS');
					}
		}
		if(estadoentrega =="ENTREGADO"){//cambiar estatus de liquidación a liquidado
			$("#loading").show();
			$.ajax({
				url: 'actualizarEstadoLiquidacion.php',
				type: "POST",
				data: "submit=&idcotizacionproducto="+idcotizacionproducto+"&estadoliquidacion="+estadoliquidacion, //Pasamos los datos en forma de array
				success: function(mensaje){
					$("#loading").hide();
					mostrarMensaje(mensaje);
					load_tablas(campoOrden,orden,cantidadamostrar,paginacion,busqueda,tipoVista);
...
function llenarSelectRuta(condicion){
		$("#idruta_ajax").html("<option value='1'>cargando...</option>");
		var estadoliquidacion = "NO LIQUIDADO";
		$.ajax({
			url: '../componentes/llenarSelectRuta.php',
			type: "POST",
			data: "submit=&condicion="+condicion+"&estadoliquidacion="+estadoliquidacion, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
				$("#idruta_ajax").html(mensaje);
			}
		});
		return false;
}
...
function actualizarObservaciones(idcotizacionproducto,no){
	var observaciones = $("#observacionesliquidacion"+no).val();
	$("#loading").show();
	$.ajax({
		url: 'actualizarObservaciones.php',
		type: "POST",
