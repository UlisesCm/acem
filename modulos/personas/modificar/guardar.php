...
if (isset($_POST['idpersona'])){
	$idpersona=htmlentities(trim($_POST['idpersona']));
	//$idpersona=mysql_real_escape_string($idpersona);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idpersona no es correcto</p>";
}
...
if($validacion){
	$resultado=$Opersona->actualizar($rfc,$razonsocial,$usocfdi,$calle,$numeroexterior,$numerointerior,$colonia,$municipio,$localidad,$estado,$pais,$cp,$email,$mensaje2,$estatus, $idpersona);
	if($resultado=="exito"){
