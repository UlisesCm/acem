...
function cambiarColor(){
	if ($('input:radio[name=estado]:checked').val()=="Pendiente"){
		$("#iconoVenta").removeClass("bg-green");
		$("#iconoVenta").addClass("bg-red");
		$(".seccionNota").hide();
		$(".seccionFecha").hide();
	}else{
		$(".seccionFecha").show();
		$("#iconoVenta").removeClass("bg-red");
		$("#iconoVenta").addClass("bg-green");
		if ($("#cdiferenciaCredito").val()!=0){
			$(".seccionNota").show();
		}else{
			$(".seccionNota").hide();
		}
