...
//    cargaAtt($CfdiRel, array("UUID"=>"A39DA66B-52CA-49E3-879B-5C05185B0EF7"));    
    ...
    for ($i=0; $i<count($Array_Cantidad); $i++){
        ...
        cargaAtt($concepto, array(
               ...
               "Importe"=>round($Array_Importe[$i],$DECIMALES)
            )
        );
		if(round(round($Array_Descuento[$i],6)*round($Array_Cantidad[$i],6),$DECIMALES)!=0){
			cargaAtt($concepto, array("Descuento"=>round(round($Array_Descuento[$i],6)*round($Array_Cantidad[$i],6),$DECIMALES)));
		}
    ...
    foreach($RespServ as $Nodo){
        ...
        if($valor_del_nodo != ""){
            ...
            foreach($comprobante as $timFis){
					...
					if($Ofactura->guardar($foliointerno,$fechaAlta,$tipocomprobante,$clasificacion,$razonsocialEmisor,$rfcEmisor,$razonsocialReceptor,$rfcReceptor,$total,$montoPagado,$estadoCFDI,$fechaPago,$formapago,"",$tim_uuid,$observaciones,$comprobantesRelacionados,$rutaEmpresaTrunca.$nombreArchivo,"activo")){
