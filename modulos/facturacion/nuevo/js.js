...
function agregarFila(tabla, elementos,lista){
		...
		nuevaFila=nuevaFila+"<td id='idreferencia"+FILAS+"' style='display:none'>0</td>";
...
function recorrerTabla(tabla,lista){
	var no, idproducto, cantidad, descripcionProducto, estiloDescripcion, unidad, precio, ieps, tasaieps, totalieps, iva, tasaiva, totaliva, monto, cantidadTicket, montoTicket, descripcionTicket, cadenaTicket, idreferencia;
	var cadena;
	...
	$('#'+tabla+' tbody tr').each(function () {
		$(this).find('td').each(function (index,valor) {
			...
			if (index==2){
				...
				descripcionProducto=$(valor).html();
			}
			if (index==3){
				unidad=$(valor).html();
			}
			if (index==4){
				cantidad=0;
				$(valor).find('input').each(function() {
					cantidad=$(this).val();
					if ($(this).val()==0){
						$(this).css('color', 'red');
					}else{
			...
			if (index==9){
				idreferencia=$(valor).html();
			}
		})
		cadena=cadena+idproducto+":::"+cantidad+":::"+precio+":::"+descuento+":::"+descripcionProducto+":::"+idreferencia+":::";
	})
	$("#"+lista).val(cadena);
...
function recalcularTotal(){
		...
		var archivo="recalcularTotal.php";
		if (TIPOCOMPROBANTE=="egreso"){
			archivo="recalcularTotalEgreso.php";
			$("#ctipocomprobante").val("E");
			$("#cuso").val("G02");
			$("#cmetodopago").val("PUE");
			$("#cmetodopago").prop("disabled",true);
			var cp=$("#idalmacen_ajax").val();
			...
			$("#cdocumentosrelacionados").attr("checked",true);
			if ($("#cdocumentosrelacionados").prop('checked') ) {
    			$(".seccionRelaciones").show();
			}else{
				$(".seccionRelaciones").hide();
				$("#filas2").html("");
...
function guardar(variables){
		...
		$.ajax({
			url: 'guardar.php',
			type: "POST",
			data: "submit=&"+variables+"&idtemporal="+idtemporal, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
				$("#botonGuardar").show();
				$("#botonSave").show();
				$(".cargando").hide();
...
function cargarDatos(){
	if (idtemporal!=0){
		...
		$(".cargando").show();
		$.ajax({
			url: '../componentes/consultarDetalle.php',
			type: "POST",
			data: "submit=&idtemporal="+idtemporal, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(respuesta){
				$("#botonGuardar").show();
				$("#botonSave").show();
				$(".cargando").hide();
...
function cargarDatosEgreso(variables){
	//alert(variables);
	$("#cformapago").val("99");
		$(".cargando").show();
		$.ajax({
			url: '../componentes/consultarDetalleEgresos.php',
			type: "POST",
...
function cargarDocumentosRelacionados(variables){
	//alert(variables);
	$("#cformapago").val("99");
		$(".cargando").show();
		$.ajax({
			url: '../componentes/consultarRelacionEgresos.php',
			type: "POST",
			data: "submit=&"+variables, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(respuesta){
				$("#botonGuardar").show();
				$("#botonSave").show();
				$(".cargando").hide();
				$("#filas2").html(respuesta);
				recorrerTabla2("tablaSalida2","listaSalida2"); // Crea la cadena de productos que se enviara a timbrado
			}
		});
...
function mostrarMensaje(mensaje){
	...
	if (res[0]=="exito"){ //Si la primer frase contiene la palabra "exito"
		...
		$("#subtituloRespuesta").html("Problema de marcado");
		$("#mensajeResuesta").html("No se han resivido datos de respuesta desde el servidor [003]");
		$("#modal-error").modal();
	}
