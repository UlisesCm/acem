...
if (isset($_POST['idasignacionvehicular'])){
	$idasignacionvehicular=htmlentities(trim($_POST['idasignacionvehicular']));
	//$idasignacionvehicular=mysql_real_escape_string($idasignacionvehicular);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idasignacionvehicular no es correcto</p>";
}
...
if($validacion){
	$resultado=$Oasignacionvehicular->actualizar($fecha,$idempleado,$idvehiculo,$observaciones, $idasignacionvehicular);
	if($resultado=="exito"){
