...
function comprobarReglas(){
	...
	if(recuperarCookie("campoOrdenZona")!=null){
		campoOrden=recuperarCookie("campoOrdenZona");
	...
	if(recuperarCookie("cantidadamostrarZona")!=null){
		cantidadamostrar=recuperarCookie("cantidadamostrarZona");
	...
	if(recuperarCookie("ordenZona")=="asc"){
		orden="ASC"
		$('#asc').attr('checked', true);
		$('#desc').attr('checked', false);
	}else if(recuperarCookie("ordenZona")=="desc"){
		orden="DESC"
	...
	if(recuperarCookie("mostrarIdzonaZona")=="si"){
		$('.Cidzona').show();
		$('#CheckIdzona').attr('checked', true);
	}else if(recuperarCookie("mostrarIdzonaZona")=="no"){
		$('.Cidzona').hide();
		$('#CheckIdzona').attr('checked', false);
	}
	//Mostrar u Ocultar Nombre
	if(recuperarCookie("mostrarNombreZona")=="si"){
		$('.Cnombre').show();
		$('#CheckNombre').attr('checked', true);
	}else if(recuperarCookie("mostrarNombreZona")=="no"){
		$('.Cnombre').hide();
		$('#CheckNombre').attr('checked', false);
	}
	//Mostrar u Ocultar Composicion
	if(recuperarCookie("mostrarComposicionZona")=="si"){
		$('.Ccomposicion').show();
		$('#CheckComposicion').attr('checked', true);
	}
	if(recuperarCookie("mostrarComposicionZona")=="no"){
		$('.Ccomposicion').hide();
		$('#CheckComposicion').attr('checked', false);
	}
	
	//Elegir el tipo de vista
	if(recuperarCookie("tipoVistaZona")=="tabla"){
		$('.tipoLista').show();
		$('.tipoTabla').hide();
		tipoVista="tabla";
	}else{
	...
	$( ".tipoTabla" ).click(function() {
    	crearCookie("tipoVistaZona", "tabla");
		tipoVista="tabla";
		$(".tipoLista").show();
		$(".tipoTabla").hide();
	...
	$( ".tipoLista" ).click(function() {
    	crearCookie("tipoVistaZona", "lista");
		tipoVista="lista";
		$(".tipoLista").hide();
		$(".tipoTabla").show();
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
...
$(document).ready(function() {
	...
	$("#campoOrden").change(function(){
		...
		crearCookie("campoOrdenZona", campoOrden);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$("#cantidadamostrar").change(function(){
		cantidadamostrar = this.value;
		crearCookie("cantidadamostrarZona", cantidadamostrar);
		load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
	});
	$( "#asc" ).click(function() {
    	if ($( "#asc" ).is(':checked')){
			crearCookie("ordenZona", "asc");
			orden="ASC"
			load_tablas(campoOrden,orden,cantidadamostrar,paginacion,"",tipoVista);
		}
	});
	...
	$( "#CheckIdzona" ).click(function() {
    	if ($( "#CheckIdzona" ).is(':checked')){
			crearCookie("mostrarIdzonaZona", "si");
			$('.Cidzona').show();
		}else{
			crearCookie("mostrarIdzonaZona", "no");
			$('.Cidzona').hide();
		}	
	});
	$( "#CheckNombre" ).click(function() {
    	if ($( "#CheckNombre" ).is(':checked')){
			crearCookie("mostrarNombreZona", "si");
			$('.Cnombre').show();
		}else{
			crearCookie("mostrarNombreZona", "no");
			$('.Cnombre').hide();
		}	
	});
	$( "#CheckComposicion" ).click(function() {
    	if ($( "#CheckComposicion" ).is(':checked')){
			crearCookie("mostrarComposicionZona", "si");
			$('.Ccomposicion').show();
		}else{
			crearCookie("mostrarComposicionZona", "no");
			$('.Ccomposicion').hide();
		}
	});
