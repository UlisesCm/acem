...
if (isset($_POST['idfolio'])){
	$idfolio=htmlentities(trim($_POST['idfolio']));
	//$idfolio=mysql_real_escape_string($idfolio);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idfolio no es correcto</p>";
}
...
if($validacion){
	$resultado=$Ofolio->actualizar($serie,$folioactual,$asignacion,$idsucursal,$estatus, $idfolio);
	if($resultado=="exito"){
