...
class Listaprecios{
 //constructor	
	...
	function importarArchivo($archivo,$idlistaprecios){
		...
		if($this->con->conectar()==true){
			...
			if(!mysqli_query($this->con->conect,"DELETE FROM precios WHERE idlistaprecios = '$idlistaprecios'")){
				$this->con->conect->rollback(); //COMMIT EN CASO DE QUE TODAS LAS TRANSACCIONES TENGAN EXITO
				return false;
			}
			...
			foreach ($xlsx->rows() as $columna){ 
				if ($con!=0){
				   $sae = $columna[0];
				   $idproducto = htmlentities(trim($columna[1]));
				   $nombre = htmlentities(trim($columna[2]));
				   //$familia = $columna[3];
				   $precio = htmlentities(trim($columna[4]));
				   $costo=$this->obtenerCosto($idproducto,$this->con->conect);
				   if ($costo!==false){
					   if ($costo!=0){
						$porcentajeutilidad=($precio/$costo);
						$porcentajeutilidad=($porcentajeutilidad*100)-100;
					   }else{
						   $porcentajeutilidad=0;
					   }
						if(!mysqli_query($this->con->conect,"INSERT INTO precios (idlistaprecios, idreferencia, descripcion,costo,porcentajeutilidad,porcentajedescuento, precio) VALUES ('$idlistaprecios','$idproducto','$nombre','$costo','$porcentajeutilidad','0','$precio')")){
							$this->con->conect->rollback(); //COMMIT EN CASO DE QUE TODAS LAS TRANSACCIONES TENGAN EXITO
							return false;
						}
				   }
	...
	function obtenerCosto($idproducto,$conexion){
		//print_r($listaCampos);
		$resultado=mysqli_query($conexion,"SELECT costo FROM productos WHERE idproducto='$idproducto'");
		if ($resultado){
