...
if (isset($_POST['modo'])){
	$modo=htmlentities(trim($_POST['modo']));
	$modo=trim($modo);
	
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo modo no es correcto</p>";
}
...
if (isset($_POST['listaRequisiciones']) && $_POST['listaRequisiciones']!=""){
	$listaRequisiciones=trim($_POST['listaRequisiciones']);
}else{
	//$validacion=false;
	//$mensaje=$mensaje."<p>Es necesario que ingrese al menos un producto en la lista de requisiciones</p>";
	$listaRequisiciones = "";
}
$monto=0;
...
if($validacion){
	...
	$resultado=$Ocompra->guardar($fecha,$hora,$serie,$folio,$idempleado,$comentarios,$estado,$monto,$lista,$modo,$listaRequisiciones);
