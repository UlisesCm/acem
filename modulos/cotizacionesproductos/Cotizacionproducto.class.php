...
class Cotizacionproducto{
 //constructor	
	...
	function comprobarCampoCliente($cliente){
		
		if($this->con->conectar()==true){
			//echo "SELECT nombre from clientes WHERE nombre = '$cliente'";
			$resultado=mysqli_query($this->con->conect,"SELECT nombre from clientes WHERE nombre = '$cliente'");
			if ($resultado){
				return true;//existe
			}else{
	...
	function guardarCliente($cliente,$nombrecomercial,$telefono,$estatus,$conexion){
		$validar=true;
		$this->globalIdCliente = $this->con->generarClave(2); /*Sincronizacion 1 */
		$consultaDet = "INSERT INTO clientes (idcliente, rfc, nombre, nic, limitecredito, diascredito, saldo, nombrecontacto, correocontacto, telefonocontacto, autorizardosis, autorizarproductos,comentarios, estatus) VALUES ('$this->globalIdCliente','-','$cliente','$nombrecomercial','0','0','0','$cliente','-','$telefono','-','-','-','$estatus')";
		//echo $consultaDet;
...
function guardar($clienteSeleccionado,$domicilioSeleccionado,$cliente,$telefono,$idgirocomercial,$tipovialidad,$calle,$noexterior,$nointerior,$colonia,$cp,$idzona,$nombrecomercial,$serie,$folio,$fecha,$hora,$estadopago,$estadofacturacion,$tipo,$subtotal,$impuestos,$total,$costodeventa,$utilidad,$observaciones,$idcliente,$idusuario,$idempleado,$idsucursal,$iddomicilio,$enviaradomicilio,$fechaentrega,$horaentregainicio,$horaentregafin,$prioridad,$coordenadas,$ciudad, $estado, $referencia,$observacionesdomicilio,$estadoentrega,$estatus,$lista,$dividirventa,$montodivision,$serieotros,$foliootros,$fechaotros,$tipootros,$montootros,$observacionesotros,$listaOtros,$impuestosotros,$subtotalotros,$idmodeloimpuestosotros,$estatusotros){
		...
		if($this->con->conectar()==true){
				...
				if($clienteSeleccionado==false){
					...
					/*if($this->comprobarCampoCliente($cliente)){
						return "clienteExiste";
					}
					else{//NO EXISTE SE GUARDA*/
						if($this->guardarCliente($cliente,$cliente,$telefono,$estatus,$this->con->conect)){//guarda el cliente nuevo
				...
				//}//fin else validación cliente repetido
	...
	function autorizaSalida($idcotizacionproducto,$observaciones){
		...
		if($this->con->conectar()==true){
			...
			$consultaDet = "UPDATE $cotizacionesproductos SET estadoentrega='ENTREGADO', observaciones='$observaciones', fechaentrega='$fecha_actual', horaentregainicio='$HoraActual' WHERE idcotizacionproducto='$idcotizacionproducto'";
			if(mysqli_query($this->con->conect,$consultaDet)){
				return "exito";
			}else{
				return "fracaso";
	...
	function mostrarA($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera, $idcliente,$iddomicilio,$idzona,$idsucursal,$tipo,$filtrarfecha,$fechainicio,$fechafin,$folio,$excel=""){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cotizacionesproductos']['consultar'])){
			return "denegado";
			exit;
	...
	function mostrarFacturacion($campoOrden, $orden, $inicial, $cantidadamostrar, $busqueda, $papelera, $idcliente, $serie, $folio, $estado, $idsucursal,$filtrarfecha,$fechainicio,$fechafin,$excel=""){
		/////PERMISOS////////////////
		if (!isset($_SESSION['permisos']['cotizacionesproductos']['consultar'])){
			return "denegado";
			exit;
		...
		if ($estado!="TODAS"){
			$consultaEstado=" AND $cotizacionesproductos.estadofacturacion='$estado'";
		}
		
		if ($serie!=""){
			$consultaSerie=" AND $cotizacionesproductos.serie='$serie'";
		}
		...
		$consulta = "SELECT 
					...
					$detallecotizacionesproductos.cfdiingreso,
					$detallecotizacionesproductos.cfdiegreso,
					$cotizacionesproductos.idcotizacionproducto,
					$cotizacionesproductos.serie,
					$cotizacionesproductos.folio,
					$cotizacionesproductos.fecha,
	...
	function guardarTemporalSimple($idcotizacionproducto,$subfolio){
		...
		if($this->con->conectar()==true){
				...
				$cfdiingreso="";
				$resultado=mysqli_query($this->con->conect,"SELECT cfdiingreso FROM detallecotizacionesproductos$idsucursal WHERE idcotizacionproducto='$idcotizacionproducto' AND subfolio='$subfolio'");
				if ($resultado){
					$extractor = mysqli_fetch_array($resultado);
					$cfdiingreso=$extractor["cfdiingreso"];
				}else{
					return "fracaso";
				}
				if ($cfdiingreso==""){
			...
			while ($filas=mysqli_fetch_array($resultado)) {
				...
				if ($tipoEliminacion=='falsa'){
					...
					if (mysqli_query($this->con->conect,"UPDATE $cotizacionesproductos SET estatus ='eliminado' WHERE idcotizacionproducto = $idcotizacionproducto AND tipo = 'COTIZACION'")){
						//BITACORA
						if ($_SESSION['bitacora']=="si"){
							$descripcionB="elimin&oacute; falsamente los registros: $dias, de la tabla cotizacionesproductos ";
				...
				if ($tipoEliminacion=='real'){
					if(mysqli_query($this->con->conect,"DELETE FROM $cotizacionesproductos WHERE idcotizacionproducto = $idcotizacionproducto AND tipo = 'COTIZACION'")){
						//BITACORA
						if ($_SESSION['bitacora']=="si"){
							$descripcionB="elimin&oacute; los registros: $dias, de la tabla cotizacionesproductos ";
