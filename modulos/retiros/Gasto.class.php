...
class Gasto{
 //constructor	
	...
	function registrarRetiros($fecha,$idcuenta,$cheque,$descripcion,$monto,$listaSalida,$listaSalidaGastos){
		...
		if($this->con->conectar()==true){
			$idretiro=$this->con->generarClave(2); /*Sincronizacion 1 */
			$consultaDet = "INSERT INTO retiros (idretiro, fecha, descripcion, monto, cheque, idcuenta, estatus) VALUES ('$idretiro','$fecha','$descripcion','$monto','$cheque','$idcuenta','activo')";
			//echo $consultaDet;
		...
		if($listaSalida!="0"){
			...
			if($this->con->conectar()==true){
				while ($con < count($arregloIdCompra)){
					$varConsulta = "UPDATE compras SET idretiro= $idretiro WHERE idcompra='$arregloIdCompra[$con]'";
					//echo $varConsulta;
		...
		if($listaSalidaGastos!="0"){
			...
			if($this->con->conectar()==true){
				while ($con < count($arregloIdGasto)){
					$varConsulta = "UPDATE gastos SET idretiro= $idretiro  WHERE idgasto='$arregloIdGasto[$con]'";
					if(mysqli_query($this->con->conect,$varConsulta)){
						//REGISTRO ACTUALIZADO
						$validar = "exito";
					}
	...
	function mostrarCompras($campoOrden, $orden, $inicial, $cantidadamostrar, $condicion, $papelera){
		...
		$consulta = "SELECT 
					...
					WHERE compras.autorizado = 'AUTORIZADO' AND idretiro = 0 
					GROUP BY idcompra 
