...
$(document).ready(function() {
	...
	llenarSelectCuentaprincipal(idcuentaprincipalSeleccionado,"");
	llenarSelectCuentasecundaria(idcuentasecundariaSeleccionado,"");
	llenarSelectProveedor(idproveedorSeleccionado,"");
...
function llenarSelectCuentaprincipal(seleccionado,condicion){
		$("#idcuentaprincipal_ajax").html("<option value='1'>cargando...</option>");
		$.ajax({
			...
			success: function(mensaje){
				$("#idcuentaprincipal_ajax").html(mensaje);
			}
		});
		return false;
}
function llenarSelectCuentasecundaria(seleccionado,condicion){
		$("#idcuentasecundaria_ajax").html("<option value='1'>cargando...</option>");
		$.ajax({
			url: '../componentes/llenarSelectCuentasecundaria.php',
			type: "POST",
			data: "submit=&condicion="+condicion+"&seleccionado="+seleccionado, //Pasamos los datos en forma de array seralizado desde la funcion de envio
			success: function(mensaje){
				$("#idcuentasecundaria_ajax").html(mensaje);
			}
		});
		return false;
}
function llenarSelectProveedor(seleccionado,condicion){
