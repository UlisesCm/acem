...
if (isset($_POST['idtraspaso'])){
	$idtraspaso=htmlentities(trim($_POST['idtraspaso']));
	//$idtraspaso=mysql_real_escape_string($idtraspaso);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idtraspaso no es correcto</p>";
}
...
if($validacion){
	$resultado=$Otraspaso->actualizar($idmovimiento,$idsucursalorigen,$idsucursaldestino,$fechasalida,$fechaentrada,$estado,$numerocomprobante,$idusuariosalida,$idusuarioentrada, $idtraspaso);
	if($resultado=="exito"){
