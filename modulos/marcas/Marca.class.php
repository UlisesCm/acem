...
class Marca{
 //constructor	
	...
	function armarConsulta($condicion,$papelera){
		if ($condicion!=""){
			if($papelera){
				$consulta="WHERE ((marcas.nombre LIKE '%".$condicion."%') OR (marcas.prefijocodigo LIKE '%".$condicion."%'))AND marcas.estatus ='eliminado'";
			}else{
				$consulta="WHERE ((marcas.nombre LIKE '%".$condicion."%') OR (marcas.prefijocodigo LIKE '%".$condicion."%'))AND marcas.estatus <>'eliminado'";
			}
		}else{
	...
	function guardar($nombre,$prefijocodigo,$estatus){
		/////PERMISOS////////////////
		...
		$idmarca=$this->con->generarClave(2); /*Sincronizacion 1 */
		
		if($this->con->conectar()==true){
			if($this->comprobarCampo("nombre",$nombre, "nuevo")){
				return "nombreExiste";
			}else if($this->comprobarCampo("prefijocodigo",$prefijocodigo, "nuevo")){
				return "prefijocodigoExiste";
			}else{
				if(mysqli_query($this->con->conect,"INSERT INTO marcas (idmarca, nombre, prefijocodigo, estatus) VALUES ('$idmarca','$nombre','$prefijocodigo','$estatus')")){
					//BITACORA
	...
	function actualizar($nombre,$prefijocodigo,$estatus,$idmarca){
		/////PERMISOS////////////////
		...
		if($this->con->conectar()==true){
			if($this->comprobarCampo("nombre",$nombre, "modificar")){
				return "nombreExiste";
			}else if($this->comprobarCampo("prefijocodigo",$prefijocodigo, "modificar")){
				return "prefijocodigoExiste";
			}else{
				if(mysqli_query($this->con->conect,"UPDATE marcas SET nombre='$nombre', prefijocodigo='$prefijocodigo', estatus='$estatus' WHERE idmarca='$idmarca'")){
					//BITACORA
					if ($_SESSION['bitacora']=="si"){
