 ...
 //constructor	
	...
	function guardar($nombre,$calle,$numero,$colonia,$cp,$ciudad,$estado,$coordenadas,$telefonocontacto,$licenciassa,$serie,$folio,$idcuentacorreo,$archivofirma,$estatus){
		...
		if($this->con->conectar()==true){
			...
			if($this->comprobarCampo("nombre",$nombre, "nuevo")){
				...
				$varConsulta = "INSERT INTO sucursales (idsucursal, nombre, calle, numero, colonia, cp, ciudad, estado, coordenadas, telefonocontacto, licenciassa, serie, folio, idcuentacorreo, archivofirma, estatus) VALUES ('$idsucursal','$nombre','$calle','$numero','$colonia','$cp','$ciudad','$estado', '$coordenadas', '$telefonocontacto','$licenciassa','$serie','$folio','$idcuentacorreo','$archivofirma','$estatus')";
	...
	function crearTablas($conexion,$idsucursal){
		...
		CREATE TABLE IF NOT EXISTS `cotizacionesproductos$idsucursal` (
		  ...
		  `estadofacturacion` varchar(12) NOT NULL,
		...
		CREATE TABLE IF NOT EXISTS `inventario$idsucursal` (
		  ...
		  `promedio` decimal(10,4) NOT NULL,
		  `saldo` decimal(10,4) NOT NULL,
		  `minimo` decimal(10,4) NOT NULL,
